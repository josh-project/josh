...
fn run_job(
    ...
    // A run can cross the high-water mark after the initial cleanup. Check
    // between jobs so one workspace revision cannot consume the VM's remaining
    // capacity before the next invocation gets a chance to reclaim it.
    crate::clean::reclaim_if_needed(transaction, graph.root().ws_tree, runtime)?;
...
mod tests {
    ...
    impl ArtifactBackend for FakeRuntime {
        ...
        fn remove_artifact(&self, name: &str, _force: bool) -> anyhow::Result<()> {
            if self.artifacts.lock().remove(name) {
                let reclaimed = self.artifact_bytes.lock().get(name).copied().unwrap_or(0);
                if let Some(status) = self.storage_status.lock().as_mut() {
                    status.used_bytes = status.used_bytes.saturating_sub(reclaimed);
                }
            }
            Ok(())
        }
    ...
    #[test]
    fn reclaims_stale_artifacts_between_jobs() {
        let dir = tempfile::tempdir().unwrap();
        gix::init_bare(dir.path()).unwrap();
        let context =
            cache::TransactionContext::new(dir.path(), Arc::new(cache::CacheStack::new()));
        let transaction = context.open().unwrap();
        let (graph, _, _, _) = graph_with_image_input(&transaction);
        let runtime = FakeRuntime::default();
        let stale = naming::output(
            gix_hash::ObjectId::from_hex(b"0123456789abcdef0123456789abcdef01234567").unwrap(),
        );
        runtime.artifacts.lock().insert(stale.clone());
        runtime.artifact_bytes.lock().insert(stale.clone(), 200);
        *runtime.storage_status.lock() = Some(StorageStatus {
            total_bytes: 1_000,
            used_bytes: 850,
        });
        *runtime.run_growth_bytes.lock() = 100;
