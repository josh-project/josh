...
pub fn get_comments(filter_spec: &str) -> anyhow::Result<String> {
    josh_filter::flang::parse::get_comments(filter_spec).map_err(|e| anyhow!("{}", e))
}

/// Invert a filter, converting josh-filter's error type to anyhow::Error
pub fn invert(filter: Filter) -> anyhow::Result<Filter> {
    josh_filter::opt::invert(filter).map_err(|e| anyhow!("{}", e))
}
