...
fn spec2(op: &Op) -> String {
    match op {
        ...
        Op::Adapt(args) => format!(":adapt={}", args.join(";")),
        Op::Link(LinkMode::Embedded) => ":link=embedded".to_string(),
        Op::Link(LinkMode::Snapshot) => ":link=snapshot".to_string(),
...
fn as_tree2(repo: &git2::Repository, op: &Op) -> JoshResult<git2::Oid> {
    ...
    match op {
        ...
        Op::Lookup2(oid) => {
            builder.insert(
                "lookup2",
                repo.blob(&oid.to_string().as_bytes())?,
                git2::FileMode::Blob.into(),
            )?;
        }
        Op::File(path) => {
        ...
        Op::Adapt(args) => {
            builder.insert(
                ...
                str_params(
                    repo,
                    args.iter().map(|x| x.as_str()).collect::<Vec<_>>().as_ref(),
                )?,
                git2::FileMode::Tree.into(),
            )?;
        }
        Op::Link(LinkMode::Embedded) => {
            builder.insert(
                "link",
                repo.blob(&"embedded".as_bytes())?,
                git2::FileMode::Blob.into(),
            )?;
        }
        Op::Link(LinkMode::Snapshot) => {
            builder.insert(
                "link",
                repo.blob(&"snapshot".as_bytes())?,
                git2::FileMode::Blob.into(),
            )?;
        }
        Op::Export => {
...
fn from_tree2(repo: &git2::Repository, tree_oid: git2::Oid) -> JoshResult<Op> {
    ...
    match name {
        ...
        "lookup2" => {
            let blob = repo.find_blob(entry.id())?;
            let oid_str = std::str::from_utf8(blob.content())?;
    ...
    let rewrite_data = match &to_op(filter) {
        ...
        Op::Link(LinkMode::Embedded) => {
            ...
            return Some(history::create_filtered_commit(
                ...
                filtered_tree.with_parents(filtered_parent_ids),
                transaction,
                filter,
            ))
            .transpose();
...
fn apply2<'a>(transaction: &'a cache::Transaction, op: &Op, x: Apply<'a>) -> JoshResult<Apply<'a>> {
    ...
    match op {
        ...
        Op::Adapt(args) => {
            ...
            for adapter in args.iter().map(|x| x.as_str()) {
                match adapter {
                    "submodules" => {
