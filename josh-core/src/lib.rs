...
pub fn get_acl(
    ...
    let users = std::fs::read_to_string(users).map_err(|_| anyhow!("failed to read users file"))?;
    let users: Users = serde_yaml::from_str(&users)
        .map_err(|err| anyhow!("failed to parse users file: {}", err))?;
    let groups =
        std::fs::read_to_string(groups).map_err(|_| anyhow!("failed to read groups file"))?;
    let groups: Groups = serde_yaml::from_str(&groups)
        .map_err(|err| anyhow!("failed to parse groups file: {}", err))?;
