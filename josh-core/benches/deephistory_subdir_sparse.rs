...
// on only a small fraction of commits, so input commits vastly outnumber output commits. That ratio
// is the realistic shape for `josh`: extracting one narrow subdirectory out of a monorepo where most
// upstream commits never touch it.
//
// Why it earns its own bench: per-commit filtering cost splits into work that scales with *input*
// commits (walking history, the sequence-number / cache bookkeeping keyed on the input oid) and work
// that scales with *output* commits (filtering changed trees, writing filtered commits). The dense
// bench keeps those roughly equal; here output is a tiny fraction of input, so it isolates the
// input-scaled overhead -- which is exactly where changes to `compute_sequence_number` and the cache
// hot path show up.
//
...
/// Expected oid of the cached bench repo's aggregate index commit -- the cache validity key. Changing
/// any build parameter above changes it and fails the strict check in `provision_repo`, which reports
...
impl SubdirBench {
    fn setup() -> anyhow::Result<Self> {
        ...
        // -- otherwise this bench would silently degenerate into the dense `deephistory_subdir` case
        // and stop measuring the input >> output regime it exists for. Checked on the largest case;
        // reset caches afterwards so nothing here warms the timed runs.
        {
...
fn deephistory_subdir_sparse(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("deephistory_subdir_sparse");
...
criterion_group!(benches, deephistory_subdir_sparse);
