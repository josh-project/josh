...
///
/// [`ExecutionBackend::start_sidecar`] returns this only once the worker is
/// reachable, so no probe address is exposed on the trait. `step_address` is
/// reachable from inside the step and injected into its environment. `id` is
/// opaque to the scheduler and used by the backend in
/// [`ExecutionBackend::stop_sidecar`].
#[derive(Debug, Clone)]
...
pub trait EnvironmentBackend {
    ...
    /// only insofar as the caller checks [`EnvironmentBackend::env_exists`] first.
...
/// Complete execution backend for `josh-compose`.
///
/// High-level orchestration uses this aggregate to ensure that environment,
/// artifact, and execution capabilities come from the same backend.
