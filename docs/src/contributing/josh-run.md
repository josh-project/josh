...
- **`josh-filter`** built and on `$PATH`. Install it first:
  ...
  ```sh
  cargo install --path ./josh-cli --features incubating --force --locked
  ```
...
This runs both the release test suite and the incubating test suite (with `--features incubating`).
