...
fn handle_link_add(
    ...
    let path_filter = josh_core::filter::to_filter(josh_core::filter::Op::Subdir(
        std::path::PathBuf::from(normalized_path),
    ));
    let export_filter =
        josh_core::filter::parse(":export").context("Failed to parse :export filter")?;
    ...
    let combined_filter = path_filter
        .chain(export_filter)
        .chain(josh_core::filter::invert(filter_obj)?);
    ...
    let initial_oid = if export_oid != git2::Oid::zero() {
        ...
        if fetched_oid == git2::Oid::zero() {
            return Err(anyhow!(
                "No valid commit for path '{}': neither local export nor remote fetch yielded a result",
                args.path
            ));
        }

        println!("Using fetched commit {}", fetched_oid);
        fetched_oid
    };
...
fn handle_link_update(
    ...
    let Some(result) = josh_link::update_links(
        ...
        println!("All {} link file(s) already up to date", link_files.len());
        return Ok(());
    };
...
fn handle_link_push(
    ...
    // Normalize path: strip leading slash(es)
    let normalized_path = args.path.trim_start_matches('/');
