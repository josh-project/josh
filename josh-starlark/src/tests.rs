...
"#;
    let filter = evaluate(script, empty_tree_oid, Arc::clone(&repo_arc)).unwrap();
...
fn create_test_repo() -> (Arc<Mutex<git2::Repository>>, git2::Oid) {
    ...
    let lib_tree_oid = {
        ...
        // Create blobs
        let utils_rs_blob = repo_guard
            .blob(b"pub fn helper() {\n    // helper\n}")
            .unwrap();

        // Create nested tree (lib/)
        let mut lib_builder = repo_guard.treebuilder(None).unwrap();
        lib_builder
            .insert("utils.rs", utils_rs_blob, 0o100644)
            .unwrap();
        lib_builder.write().unwrap()
    };

    let src_tree_oid = {
        let repo_guard = repo_arc.lock().unwrap();
        let main_rs_blob = repo_guard
            .blob(b"fn main() {\n    println!(\"Hello\");\n}")
            .unwrap();

        // Create src/ tree
        let mut src_builder = repo_guard.treebuilder(None).unwrap();
        src_builder
            .insert("main.rs", main_rs_blob, 0o100644)
            .unwrap();
        src_builder.insert("lib", lib_tree_oid, 0o040000).unwrap();
        src_builder.write().unwrap()
    };

    let root_tree_oid = {
        let repo_guard = repo_arc.lock().unwrap();
        let readme_blob = repo_guard
            .blob(b"# Project\nThis is a test project.")
            .unwrap();

        // Create root tree
        let mut root_builder = repo_guard.treebuilder(None).unwrap();
        root_builder
...
filter = filter.subdir("src")
"#;
    let filter = evaluate(script, root_tree_oid, Arc::clone(&repo_arc)).unwrap();
    let filter_spec = spec(filter);
    assert_eq!(filter_spec, ":/src");
}
