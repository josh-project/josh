    ...
    pub fn process(
        ...
        let cands = 'cands: {
            if let Some(parent_state) = parent {
                ...
                if !self.spine_paths.is_empty() && parent_state.roots == roots {
                    // Identical non-empty roots: same candidate paths, only blobs may differ.
                ...
                } else if !self.spine_paths.is_empty() && self.conjunctive {
...
mod tests {
    ...
    fn change_sweep_matches_brute_force() {
        ...
        for pattern in ["needle", r"needle (?:one|two)|nothing", "i"] {
