...
fn from_tree2(repo: &git2::Repository, tree_oid: git2::Oid) -> anyhow::Result<Op> {
    ...
    let entry = tree.get(0).ok_or_else(|| anyhow!("Empty tree"))?;
    let name = entry.name().ok_or_else(|| anyhow!("Entry has no name"))?;
    ...
    match name {
        ...
        "link" => {
            ...
            let mode_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("link: missing mode"))?
                    .id(),
            )?;
            Ok(Op::Link(
        ...
        "adapt" => {
            ...
            let mode_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("adapt: missing mode"))?
                    .id(),
            )?;
            Ok(Op::Adapt(
                std::str::from_utf8(mode_blob.content())?.to_string(),
        ...
        "hook" => {
            ...
            let hook_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("hook: missing hook name"))?
                    .id(),
            )?;
        ...
        "author" => {
            ...
            let name_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("author: missing name"))?
                    .id(),
            )?;
            let email_blob = repo.find_blob(
                inner
                    .get_name("1")
                    .ok_or_else(|| anyhow!("author: missing email"))?
                    .id(),
            )?;
        ...
        "committer" => {
            ...
            let name_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("committer: missing name"))?
                    .id(),
            )?;
            let email_blob = repo.find_blob(
                inner
                    .get_name("1")
                    .ok_or_else(|| anyhow!("committer: missing email"))?
                    .id(),
            )?;
            let name = std::str::from_utf8(name_blob.content())?.to_string();
            let email = std::str::from_utf8(email_blob.content())?.to_string();
        ...
        "message" => {
            ...
            let fmt_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("message: missing fmt string"))?
                    .id(),
            )?;
            let regex_blob = repo.find_blob(
                inner
                    .get_name("1")
                    .ok_or_else(|| anyhow!("message: missing regex"))?
                    .id(),
            )?;
        ...
        "subdir" => {
            ...
            let path_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("subdir: missing path"))?
                    .id(),
            )?;
        ...
        "prefix" => {
            ...
            let path_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("prefix: missing path"))?
                    .id(),
            )?;
            let path = std::str::from_utf8(path_blob.content())?;
        ...
        "file" => {
            ...
            let dest_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("file: missing destination path"))?
                    .id(),
            )?;
            let source_blob = repo.find_blob(
                inner
                    .get_name("1")
                    .ok_or_else(|| anyhow!("file: missing source path"))?
                    .id(),
            )?;
        ...
        "embed" => {
            ...
            let path_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("embed: missing path"))?
                    .id(),
            )?;
            let path = std::str::from_utf8(path_blob.content())?;
        ...
        "pattern" => {
            ...
            let pattern_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("pattern: missing pattern"))?
                    .id(),
            )?;
        ...
        "workspace" => {
            ...
            let path_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("workspace: missing path"))?
                    .id(),
            )?;
            let path = std::str::from_utf8(path_blob.content())?;
        ...
        "stored" => {
            ...
            let path_blob = repo.find_blob(
                inner
                    .get_name("0")
                    .ok_or_else(|| anyhow!("stored: missing path"))?
                    .id(),
            )?;
            let path = std::str::from_utf8(path_blob.content())?;
        ...
        "starlark" => {
            ...
            let path_tree = inner
                .get_name("0")
                .ok_or_else(|| anyhow!("starlark: missing path"))?;
            let path_blob = repo.find_blob(
                repo.find_tree(path_tree.id())?
                    .get_name("0")
                    .ok_or_else(|| anyhow!("starlark: missing path blob"))?
                    .id(),
            )?;
            let path = std::str::from_utf8(path_blob.content())?;
            let filter_tree = repo.find_tree(
                inner
                    .get_name("1")
                    .ok_or_else(|| anyhow!("starlark: missing filter"))?
                    .id(),
            )?;
        ...
        "compose" => {
            ...
            for i in 0..compose_tree.len() {
                let compose_entry = compose_tree
                    .get(i)
                    .ok_or_else(|| anyhow!("compose: missing entry"))?;
        ...
        "subtract" => {
            ...
            if subtract_tree.len() == 2 {
                let a_tree = repo.find_tree(
                    subtract_tree
                        .get_name("0")
                        .ok_or_else(|| anyhow!("subtract: missing 0"))?
                        .id(),
                )?;
                let b_tree = repo.find_tree(
                    subtract_tree
                        .get_name("1")
                        .ok_or_else(|| anyhow!("subtract: missing 1"))?
                        .id(),
                )?;
                ...
                for i in 0..chain_tree.len() {
                    let filter_tree = repo.find_tree(
                        chain_tree
                            ...
                            .ok_or_else(|| anyhow!("chain: missing {}", i))?
                            .id(),
                    )?;
                    let filter = from_tree2(repo, filter_tree.id())?;
                    filters.push(to_filter(filter));
        ...
        "exclude" => {
            ...
            if exclude_tree.len() == 1 {
                let filter_tree = repo.find_tree(
                    exclude_tree
                        .get_name("0")
                        .ok_or_else(|| anyhow!("exclude: missing 0"))?
                        .id(),
                )?;
                let filter = from_tree2(repo, filter_tree.id())?;
        ...
        "pin" => {
            ...
            if pin_tree.len() == 1 {
                let filter_tree = repo.find_tree(
                    pin_tree
                        .get_name("0")
                        .ok_or_else(|| anyhow!("pin: missing 0"))?
                        .id(),
                )?;
                let filter = from_tree2(repo, filter_tree.id())?;
        ...
        "rev" => {
            ...
            for i in 0..rev_tree.len() {
                let rev_entry = rev_tree
                    .get_name(&i.to_string())
                    .ok_or_else(|| anyhow!("rev: missing entry"))?;
                let inner_tree = repo.find_tree(rev_entry.id())?;
                let key_blob = repo.find_blob(
                    inner_tree
                        .get_name("o")
                        .ok_or_else(|| anyhow!("rev: missing key"))?
                        .id(),
                )?;
                let filter_tree = repo.find_tree(
                    inner_tree
                        .get_name("f")
                        .ok_or_else(|| anyhow!("rev: missing filter"))?
                        .id(),
                )?;
                ...
                let (match_op, lazy_ref) = if key == "_" {
                    ...
                    (
                        ...
                        LazyRef::parse(ref_str).map_err(|e| anyhow!(e))?,
                    )
                } else if let Some(ref_str) = key.strip_prefix('<') {
                    (
        ...
        "unapply" => {
            ...
            let unapply_entry = concat_tree
                .get(0)
                .ok_or_else(|| anyhow!("concat: missing entry"))?;
            let inner_tree = repo.find_tree(unapply_entry.id())?;
            let key_blob = repo.find_blob(
                inner_tree
                    .get_name("o")
                    .ok_or_else(|| anyhow!("concat: missing key"))?
                    .id(),
            )?;
            let filter_tree = repo.find_tree(
                inner_tree
                    .get_name("f")
                    .ok_or_else(|| anyhow!("concat: missing filter"))?
                    .id(),
            )?;
            let key = std::str::from_utf8(key_blob.content())?;
            let filter = from_tree2(repo, filter_tree.id())?;
            Ok(Op::Unapply(
                LazyRef::parse(&key).map_err(|e| anyhow!(e))?,
                to_filter(filter),
            ))
        }
        "squash" => {
        ...
        "regex_replace" => {
            ...
            for i in 0..regex_replace_tree.len() {
                let regex_entry = regex_replace_tree
                    .get(i)
                    .ok_or_else(|| anyhow!("regex_replace: missing entry"))?;
                let inner_tree = repo.find_tree(regex_entry.id())?;
                let regex_blob = repo.find_blob(
                    inner_tree
                        .get_name("p")
                        .ok_or_else(|| anyhow!("regex_replace: missing pattern"))?
                        .id(),
                )?;
                let replacement_blob = repo.find_blob(
                    inner_tree
                        .get_name("r")
                        .ok_or_else(|| anyhow!("regex_replace: missing replacement"))?
                        .id(),
                )?;
                let regex_str = std::str::from_utf8(regex_blob.content())?;
        ...
        "meta" => {
            ...
            let filter_tree = repo.find_tree(
                meta_tree
                    .get_name("0")
                    .ok_or_else(|| anyhow!("meta: missing filter tree"))?
                    .id(),
            )?;
            ...
            for i in 0..meta_tree.len() {
                let meta_entry = meta_tree
                    .get(i)
                    .ok_or_else(|| anyhow!("meta: missing metadata entry"))?;
                let meta_key = meta_entry
                    .name()
                    .ok_or_else(|| anyhow!("meta: missing metadata key"))?;
