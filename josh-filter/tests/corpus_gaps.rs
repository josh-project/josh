...
/// egg has no general common_pre, so it leaves the branches un-factored — the
/// same tree, just structurally wider (node cost ties here, but the form differs).
#[test]
fn workspace_common_prefix_factor() -> anyhow::Result<()> {
    ...
    egg:  :[:/a/b:prefix=x,:/a/c:prefix=y] (cost 9)
    "#);
    Ok(())
}
