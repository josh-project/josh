# Merge Queue Implementation Progress

| Step | Status | Commit | Description |
|------|--------|--------|-------------|
| [STEP_1.md](STEP_1.md) | completed | `d2ead61c` | Remove `fetch`/`step`/`push` CLI subcommands |
| [STEP_2.md](STEP_2.md) | completed | — | Add GraphQL query to list open PRs |
| [STEP_3.md](STEP_3.md) | completed | `b7dbf833` | Add GraphQL query to fetch PR reviews |
| [STEP_4.md](STEP_4.md) | completed | `934f5b44` | Add GraphQL mutation to post PR comment |
| [STEP_5.md](STEP_5.md) | in_progress | — | Add `CandidatePr` model and extend `CqActorState` |
| [STEP_6.md](STEP_6.md) | pending | — | Handle `PullRequest` and `Push` webhook events |
| [STEP_7.md](STEP_7.md) | pending | — | Implement `handle_fetch()` |
| [STEP_8.md](STEP_8.md) | pending | — | Implement candidate evaluation |
| [STEP_9.md](STEP_9.md) | pending | — | Implement `handle_step()` |
| [STEP_10.md](STEP_10.md) | pending | — | Wire actor loop with tick timer into `serve` |
| [STEP_11.md](STEP_11.md) | pending | — | Split `cq.rs` into modules |
