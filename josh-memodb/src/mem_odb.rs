...
//! ([`MemOdb::flush`]), and at process exit ([`crate::flush_all`]).
//!
...
pub struct MemOdb {
    ...
    /// concurrently. The flusher thread serialises its own jobs, but [`crate::flush_all`] packs
...
impl MemOdb {
    ...
    pub fn contains(&self, id: &gix_hash::oid) -> bool {
        self.inner.read().unwrap().map.contains_key(id)
    ...
    pub(crate) fn pack_to_disk(self: &Arc<Self>) -> Result<(), git2::Error> {
        ...
        let _packing = self.pack_lock.lock().unwrap();
