...
/// Pack every registered store to disk, inline. Call at the end of a binary's `main`: stores
/// outlive transactions, so the process is what ends them. Losing one costs recomputation rather
/// than correctness (cache hits are guarded by an existence probe), but anything inspecting the
/// repository afterwards expects the objects to be there.
pub fn flush_all() {
