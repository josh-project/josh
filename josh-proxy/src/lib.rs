...
pub fn process_repo_update(repo_update: RepoUpdate) -> josh::JoshResult<String> {
    ...
    for (refname, (old, new)) in repo_update.refs.iter() {
        ...
        let author = if let Some(p) = push_options.get("change-author") {
            p.to_string()
        } else {
            "".to_string()
        };
        ...
        let to_push = if let Some(change_ids) = change_ids {
            let mut v = vec![(
                baseref.replacen("refs/heads/", &format!("refs/heads/{}/heads/", author), 1),
                oid_to_push,
            )];
...
fn change_ids_to_refs(
    ...
    change_ids: Vec<((String, Option<String>), git2::Oid)>,
) -> josh::JoshResult<Vec<(String, git2::Oid)>> {
    let mut seen = vec![];
    let mut change_ids = change_ids;
    change_ids.retain(|((a, _), _)| *a == change_author);
    if change_author == "" {
        return Err(josh::josh_error(
            "Push option 'changes-author' needs to be set",
        ));
    };

    for ((_, id), sha) in change_ids.iter() {
        if let Some(id) = id {
            if id.contains('/') {
