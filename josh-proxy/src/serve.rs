...
/// Get git capabilities by invoking git http-backend
pub fn git_list_capabilities(
    ...
    let mut peekable = StreamingPeekableIter::new(cursor, &[], false);
        ...
        let capabilities: Vec<String> = caps
            ...
            .map(|cap| {
                ...
                if let Some(value) = cap.value() {
                    // Strip trailing newline from value if present
                    let value_str = value.to_string();
                    let trimmed = value_str.trim_end_matches('\n');
                    format!("{}={}", name, trimmed)
                } else {
                    name
                }
...
/// Encode info/refs response
pub fn encode_info_refs_response(
    refs: &[(String, String)],
    capabilities: &[String],
    direction: CapabilitiesDirection,
    head_ref: Option<&str>,
    ...
    // Filter out refs with null OID (unborn branches)
    const NULL_OID: &str = "0000000000000000000000000000000000000000";
    let filtered_refs: Vec<_> = refs
        .iter()
        .filter(|(oid, _)| oid != NULL_OID)
        .cloned()
        .collect();

    if !filtered_refs.is_empty() {
        // Sort refs: HEAD first, then others lexicographically
        let mut sorted_refs = filtered_refs;
        sorted_refs.sort_by(|a, b| match (&a.1 == "HEAD", &b.1 == "HEAD") {
            (true, false) => std::cmp::Ordering::Less,
            (false, true) => std::cmp::Ordering::Greater,
            _ => a.1.cmp(&b.1),
        });

        // Build capabilities list with symref if HEAD is present
        let mut caps = capabilities.to_vec();
        if let Some(head_ref_str) = head_ref {
            if sorted_refs.iter().any(|(_, refname)| refname == "HEAD") {
                caps.push(format!("symref=HEAD:{}", head_ref_str));
            }
        }
