...
impl<'a> Scratch<'a> {
        ...
        -> Result<Oid, Error>
    {
        ...
        let modules_tree = {
            let mut builder =
                try!(self.repo.treebuilder(Some(&self.subtree(master_tree, parent_path).unwrap())));
    ...
    -> Result<(), Error>
{
    ...
    for module in scratch.module_paths(&format!("{}",newrev)) {
        ...
        let commit = scratch.repo.revparse_single(&format!("initial_{}",module))
    ...
    -> Result<(), Error>
{
    ...
    for module in scratch.module_paths(newrev) {
        println!(" ####### prepare commit from scratch.repo to module {}", &module);
        let module_master_commit_obj = scratch.tracking(&module, "master");
        let parents = vec!(
            module_master_commit_obj.as_commit().expect("could not get commit from obj")
        );

        println!("\tpath for module in central repository... => {:?}", module);

        // new tree is sub-tree of complete central tree
        let old_tree = try!(parents[0].tree());
        let new_tree = try!(
            scratch.repo.find_tree(try!(
                central_tree.get_path(&Path::new(&module))).id()
            )
        );

        // if sha1's are equal the content is equal
        if new_tree.id() != old_tree.id() {
            println!("\tmodule repository for {} is behind, => updating", &module);
