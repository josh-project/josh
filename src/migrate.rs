...
pub fn module_review_upload(project: &str,
                            ...
                            central_name: &str) -> Result<(), git2::Error> {
...
fn setup_tmp_repo(remote_addr: &str,
                  ...
                  check: &Fn(&str) -> Result<(), git2::Error>,
                  remote_module_url: &Fn(&str) -> String,
                  module_path_prefix: &str) -> Result<Repository, git2::Error> {
    ...
    for module in modules.iter() {
        ...
        if !repo.find_remote(&remote_name).is_ok() {
            try!(repo.remote(&remote_name, &remote_module_url(&format!("{}/{}.git", module_path_prefix, module))));
        }
    }
...
fn module_to_subfolder(path: &Path,
                       module_tree: &Tree,
                       master_tree: &Tree) -> Result<Oid, git2::Error> {
...
fn push_from_tmp(tmp_repo: &Repository,
                 ...
                 repo: &str,
                 to: &str,
                 remote_module_url: &Fn(&str) -> String) -> Result<String, git2::Error> {
