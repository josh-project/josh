...
///
/// Every object is written unconditionally; batching or flushing of writes is
/// the store's business. Each tree's entries are emitted in canonical git
/// order -- filename bytes, with '/' appended to directory names for the
/// comparison, which is exactly [`gix_object::tree::Entry`]'s `Ord`.
///
...
/// Read the object `root` and everything it references from `source` into a
...
/// plain data, so anything else would be silently mutated on a later write.
///
/// Entry names are taken over as-is -- decoding percent-escapes is the
/// deserializer's business. Shared subtrees are materialized repeatedly; the
