...
The prysk-updated `.t` files (showing diffs for failures) are stored in the `out_<WS_TREE>` podman volume under `tests/`, **not** in the working directory. `josh compose run` prints the `WS_TREE` SHA near the top of its output. To inspect the test results:
...
The output of each run is stored in a podman volume named `out_<WS_TREE_SHA>`. If a volume for the current tree SHA already exists, `josh compose run` skips execution entirely and just re-exports it.
...
The build dependency (`josh compose run`) has its own separate output volume (prefixed differently). Check `podman volume ls` to see all volumes.

### Iterating on a failing test

1. Edit the `.t` test file or the relevant source code.
2. Re-run `josh compose run . :+ws/test` — the changed working tree produces a new SHA, so the cache is bypassed automatically.
